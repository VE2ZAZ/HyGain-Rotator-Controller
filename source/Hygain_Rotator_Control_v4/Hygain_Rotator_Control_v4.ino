/*
 ...
 ( )( )(  _)(__ \(_  ) (  ) (_  )   / ) ( )( )(  ) (__ \(  )( \/\/ )
  ...
  Modern HyGain Rotator Controller, Version 4, January 2025.
  ...
  Version Tracking:
  =================
  - Version 4 - 01/2025: Adds the option to control both North-Stop (South-centered) or South-Stop (North-Centered) rotators. 
    Permanently short Pins 2 and 3 of Pico to enable South-Stop.
...
boolean south_stop = false;
// unsigned int pot_cursor = 0;
// unsigned int pot_maxval = 0;
// unsigned int pot_minval = 0;
// unsigned int target_pot_cursor = 0;
// unsigned int previous_pot_cursor = 0;
int pot_cursor = 0;
int pot_maxval = 0;
int pot_minval = 0;
double pot_cursor_temp = 0;
double pot_maxval_temp = 0;
double pot_minval_temp = 0;
int target_pot_cursor = 0;
int previous_pot_cursor = 0;
float pot_one_degree_val = 0;
...
void setup() 
{
  ...
  pinMode(NORTH_SOUTH_STOP_PIN,INPUT_PULLUP);             // Initialize the North/South-stop pin as input with pullup
  south_stop = !(digitalRead(NORTH_SOUTH_STOP_PIN));      // Detect whether the controller is attached to a North-stop or a South-stop rotator. A ground on the pin means South-Stop
  ...
  tft.setCursor(10,90);
  tft.print("  Version 4 - 01/2025");
  tft.setCursor(10,110);
  tft.print("   by VE2ZAZ - VA2IW");
  tft.setCursor(10,130);
  tft.print("  https://ve2zaz.net");
  tft.setCursor(10,170);
  tft.print("Rotator Cnfg: ");
  if (south_stop) tft.print("South-Stop");
  else tft.print("North-Stop");
...
void loop() 
{
  ...
  { // Yes
    ...
    if (rotation_req)                                                 // Is there a rotation request?
    {  // Yes
      if (!south_stop) target_pot_cursor = round(float(pot_minval) + float((360 - target_heading) * pot_one_degree_val));   // North-stop case. Pot is inverted on tailtwister
      else target_pot_cursor = (round(float(pot_minval) + float((((180 - target_heading) + 360) % 360) * pot_one_degree_val)));   // South-stop case

      if ((!south_stop) && ((current_heading + SLOWDOWN_RESIDUAL_ROTATION) < target_heading)       // This condition valid for North stop
          || ((south_stop) && ((current_heading + SLOWDOWN_RESIDUAL_ROTATION) < target_heading) && (current_heading <= 180) && (target_heading <= 180))       // This condition valid for South stop
          || ((south_stop) && ((current_heading + SLOWDOWN_RESIDUAL_ROTATION) < target_heading) && (current_heading > 180) && (target_heading > 180))       // This condition valid for South stop
          || ((south_stop) && ((current_heading + SLOWDOWN_RESIDUAL_ROTATION) > target_heading) && (current_heading > 180) && (target_heading <= 180))) // This condition valid for South stop
       {
      ...
      else if ((!south_stop) && ((current_heading - SLOWDOWN_RESIDUAL_ROTATION) > target_heading)   // This condition valid for Northstop
              || ((south_stop) && ((current_heading - SLOWDOWN_RESIDUAL_ROTATION) > target_heading) && (current_heading <= 180) && (target_heading <= 180))   // This condition valid for South stop
              || ((south_stop) && ((current_heading - SLOWDOWN_RESIDUAL_ROTATION) > target_heading) && (current_heading > 180) && (target_heading > 180))   // This condition valid for South stop
              || ((south_stop) && ((current_heading - SLOWDOWN_RESIDUAL_ROTATION) < target_heading) && (current_heading <= 180) && (target_heading > 180))) // This condition valid for South stop  
      {
    ...
    {  // Already in rotation phase 
      if ((!digitalRead(ENCODER_PUSHBUTTON))                                                              // Forced stop via encoder pushbutton?
          || ((cw_rotation) && ((pot_cursor - int(pot_one_degree_val * SLOWDOWN_RESIDUAL_ROTATION)) <= target_pot_cursor))       // or heading reached during CW rotation?
          || ((!cw_rotation) && ((pot_cursor + int(pot_one_degree_val * SLOWDOWN_RESIDUAL_ROTATION)) >= target_pot_cursor))       // or heading reached during CCW rotation?
    ...
    if (!ongoing_slowdown)                                          // Not in slowdown phase?
    {
      ...
      { // Beginning of waiting period before checking for rotation
        ...
        previous_pot_cursor = pot_cursor;
      ...
      {  //Yes
        ...
        if (float(abs(int(pot_cursor - previous_pot_cursor)) / pot_one_degree_val) < 3)  // Must see a minimum of 3 degree change
...
void loop1()
{
  ...
  {   // Yes
    ...
    {
      pot_cursor_temp += analogRead(POT_CURSOR);                                       // Accumulate ADC readings of pot cursor voltage
      pot_maxval_temp += analogRead(POT_MAX);                                          // Accumulate ADC readings of pot maximum voltage
      pot_minval_temp += analogRead(POT_MIN);                                          // Accumulate ADC readings of pot minimum voltage
      delay(1);                                                                   // 1 ms delay
    }
    pot_cursor = round(float(pot_cursor_temp) / POT_NUMBER_SAMPLE);                    // Average calculation of pot cursor voltage
    pot_maxval = round(float(pot_maxval_temp) / POT_NUMBER_SAMPLE);                    // Average calculation of pot maximum voltage
    pot_minval = round(float(pot_minval_temp) / POT_NUMBER_SAMPLE);                    // Average calculation of pot minimum voltage
    pot_one_degree_val = float(pot_maxval - pot_minval) / 360;
    instant_heading = 360 - round(float(pot_cursor - pot_minval) / float(pot_maxval - pot_minval) * 360);    // Instantaneous heading calculation. Warning: rotator voltage distribution is inverted compared to heading.
    ...
    if (south_stop)
    {
      if (instant_heading > 180) instant_heading -= 180;
      else instant_heading += 180;
      if ((instant_heading == 180) && (pot_cursor > (pot_maxval - 100))) instant_heading = 181;   // Spacial case to cover otherwise the controller will be stuck (180 degrees position can only be at one end)
    } 
